...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  const auto find_field = [&](const std::string &name) -> const sensor_msgs::msg::PointField * {
  ...
  if (!msg->is_bigendian && x_field != nullptr && y_field != nullptr && z_field != nullptr &&
      ...
      timestamp_field->count == 1)
  {
    ...
    const auto read_float = [&](const uint8_t *point, uint32_t offset) -> float {
      float value = 0.0f;
      std::memcpy(&value, point + offset, sizeof(float));
      return value;
    };
    const auto read_double = [&](const uint8_t *point, uint32_t offset) -> double {
      double value = 0.0;
      std::memcpy(&value, point + offset, sizeof(double));
      return value;
    };

    const uint8_t *data = msg->data.data();
    double min_stamp = std::numeric_limits<double>::infinity();
    uint32_t valid_stamp_count = 0;
    for (uint32_t i = 0; i < point_count; ++i)
    {
      const uint8_t *point = data + static_cast<size_t>(i) * msg->point_step;
      const float x = read_float(point, x_field->offset);
      const float y = read_float(point, y_field->offset);
      const float z = read_float(point, z_field->offset);
      const double range = static_cast<double>(x) * x + static_cast<double>(y) * y + static_cast<double>(z) * z;
      const double stamp = read_double(point, timestamp_field->offset);
      if (!std::isfinite(x) || !std::isfinite(y) || !std::isfinite(z) || range <= blind * blind ||
    ...
    if (valid_stamp_count > 0 && std::isfinite(min_stamp))
    {
      double stamp_to_sec = 1.0;
      switch (time_unit)
      {
        case SEC:
          stamp_to_sec = 1.0;
      ...
      scan_start_time = min_stamp * stamp_to_sec;
      has_scan_start_time = true;
      given_offset_time = true;
      pl_surf.reserve(point_count / std::max(point_filter_num, 1));
      ...
      for (uint32_t i = 0; i < point_count; ++i)
      {
        ...
        added_pt.y = y;
        added_pt.z = z;
        added_pt.intensity = read_float(point, intensity_field->offset);
        added_pt.curvature = static_cast<float>((stamp - min_stamp) * time_unit_scale);
...
void Preprocess::isaac_os0_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  if (msg->is_bigendian || x_field == nullptr || y_field == nullptr || z_field == nullptr ||
      ...
      z_field->datatype != sensor_msgs::msg::PointField::FLOAT32 ||
      intensity_field->datatype != sensor_msgs::msg::PointField::FLOAT32 ||
      timestamp_field->datatype != sensor_msgs::msg::PointField::UINT32 ||
      timestamp_field->count < 2)
  {
    default_handler(msg);
    return;
  }
  ...
  const auto read_u32 = [&](const uint8_t *point, uint32_t offset) -> uint32_t {
    uint32_t value = 0;
    std::memcpy(&value, point + offset, sizeof(uint32_t));
    return value;
  };
  const auto read_timestamp_ns = [&](const uint8_t *point) -> uint64_t {
    const uint32_t lo = read_u32(point, timestamp_field->offset);
    const uint32_t hi = read_u32(point, timestamp_field->offset + sizeof(uint32_t));
    return (static_cast<uint64_t>(hi) << 32) | static_cast<uint64_t>(lo);
  };

  uint64_t min_stamp_ns = std::numeric_limits<uint64_t>::max();
  uint64_t max_stamp_ns = 0;
  uint32_t valid_stamp_count = 0;
  const uint8_t *data = msg->data.data();
  for (uint32_t i = 0; i < point_count; ++i)
  {
    ...
    const uint64_t stamp_ns = read_timestamp_ns(point);
    if (stamp_ns == 0)
    {
      continue;
    }
    min_stamp_ns = std::min(min_stamp_ns, stamp_ns);
    max_stamp_ns = std::max(max_stamp_ns, stamp_ns);
    valid_stamp_count++;
  }

  if (valid_stamp_count == 0 || min_stamp_ns == std::numeric_limits<uint64_t>::max())
  {
    default_handler(msg);
    return;
  }

  scan_start_time = static_cast<double>(min_stamp_ns) * 1.0e-9;
  has_scan_start_time = true;
  pl_surf.reserve(point_count / std::max(point_filter_num, 1));
  ...
  for (uint32_t i = 0; i < point_count; ++i)
  {
    ...
    added_pt.z = z;
    added_pt.intensity = read_float(point, intensity_field->offset);
    added_pt.curvature = static_cast<float>((read_timestamp_ns(point) - min_stamp_ns) * 1.0e-6);
    pl_surf.push_back(std::move(added_pt));
  }
}
