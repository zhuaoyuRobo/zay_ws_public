...
inline double pointTimeValueScaleToSeconds(
  const sensor_msgs::msg::PointField & field,
  double raw_value)
{
  const bool floating_timestamp =
    field.name == "timestamp" &&
    (field.datatype == sensor_msgs::msg::PointField::FLOAT32 ||
    field.datatype == sensor_msgs::msg::PointField::FLOAT64);

  // livox_ros_driver2 publishes Unix epoch nanoseconds in a FLOAT64 timestamp field.
  // Keep ordinary floating-point timestamps in seconds compatible with other drivers.
  if (floating_timestamp && std::abs(raw_value) >= 1.0e17) {
    return 1.0e-9;
  }
  ...
  double * time_seconds)
{
  ...
  *time_seconds = raw_value * pointTimeValueScaleToSeconds(field, raw_value);
  return true;
}
