...
void test_float64_timestamp_detects_livox_epoch_nanoseconds()
{
  auto cloud = make_sensor_cloud();
  cloud.point_step = 24;
  cloud.row_step = cloud.point_step * cloud.width;
  cloud.fields.push_back(
    make_field("timestamp", 16, sensor_msgs::msg::PointField::FLOAT64));
  cloud.data.assign(cloud.row_step, 0);

  const double scan_start_ns = 1787027515321883136.0;
  const double scan_end_ns = scan_start_ns + 102400000.0;
  write_value<double>(cloud.data, 16, scan_start_ns);
  write_value<double>(cloud.data, 40, scan_end_ns);

  const auto * timestamp_field = ll::findPointTimeField(cloud.fields);
  assert(timestamp_field != nullptr);

  double time_seconds = 0.0;
  assert(ll::readPointTimeSeconds(cloud.data.data(), *timestamp_field, &time_seconds));
  assert(std::abs(time_seconds - scan_start_ns * 1.0e-9) < 1.0e-6);

  const ll::PointTimeRange range = ll::computePointTimeRangeSeconds(cloud);
  assert(range.valid);
  assert(std::abs(range.durationSec() - 0.1024) < 1.0e-6);
}
...
int main()
{
  ...
  test_float64_timestamp_detects_livox_epoch_nanoseconds();
